...
- **Logical replication (WAL).** flusso consumes a logical replication slot. The slot is
  **created automatically** if it does not exist. The **publication** is also **managed
  ...
  While the indexed tables are idle, a running flusso still advances the slot from the
  server's keepalives, so WAL generated by unrelated tables is not retained.
