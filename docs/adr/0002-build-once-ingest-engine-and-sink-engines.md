...
- Reindex is per sink and restarts only that sink's engine. Readers keep zero downtime from the alias swap; #6 shrinks to the writer stall on one sink.
...
- The daemon becomes a supervisor: it wires lanes, restarts a failed engine with backoff while the others keep running, and exposes operations (reindex) while engines expose primitives and the binary owns transport. A stalled sink engine pins WAL for the shared slot until it recovers; the per-sink lag metric is the alarm.
