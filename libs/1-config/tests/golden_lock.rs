...
fn golden_lock_matches_the_committed_bytes() {
    let compiled = config::compile(golden_dir().join("flusso.toml")).unwrap();
    let actual = config::to_bytes(&compiled).unwrap();
...
fn serialization_is_deterministic_and_roundtrips() {
    ...
    let first = config::to_bytes(&compiled).unwrap();
    let second = config::to_bytes(&compiled).unwrap();
    ...
    let config = config::from_bytes(&first).unwrap();
    let rewrapped = config::Compiled {
        format_version: config::FORMAT_VERSION,
        config,
    };
    let third = config::to_bytes(&rewrapped).unwrap();
...
fn legacy_binary_lock_is_diagnosed() {
    ...
    let err = config::from_bytes(&legacy).unwrap_err();
    assert!(matches!(err, config::CompileError::LegacyFormat));
...
fn future_format_version_is_rejected() {
    ...
    match err {
        config::CompileError::VersionMismatch { got, expected } => {
            assert_eq!(got, 200);
            assert_eq!(expected, config::FORMAT_VERSION);
        }
