//! The `*.schema.yml` parser: parse, then convert + validate into the kernel's
//! [`IndexSchema`](kernel::IndexSchema).
//!
//! 1. **Parse**: [`SchemaYaml`] deserializes the file; each field's type tag
//!    (`- keyword: email`, `- has_many: orders`, `- count: orderCount`) selects
//!    the body shape it parses into (see [`Field`]). The declared `version` is
//!    checked against [`SUPPORTED_VERSIONS`].
//! 2. **Convert**: `TryFrom<SchemaYaml>` validates identifiers and the arity
//!    rules YAML alone cannot express (a join takes exactly the key its verb
//!    implies, `sum`/`min`/`max` need a `column` and `value_type`, `between`
//!    takes two values, `geo` takes `lat`+`lon` or one `column`, enum variants
//!    are unique) and lifts the result into the kernel model.
...
pub enum ConversionError {
    ...
    TableName(#[from] kernel::TableNameError),
    #[error("invalid column name: {0}")]
    ColumnName(#[from] kernel::ColumnNameError),
    #[error("invalid database schema name: {0}")]
    DatabaseSchema(#[from] kernel::DatabaseSchemaError),
...
impl TryFrom<SchemaYaml> for kernel::IndexSchema {
    type Error = ConversionError;

    fn try_from(yaml: SchemaYaml) -> Result<Self, Self::Error> {
        use kernel::common::{ColumnName, TableName};

        let table = TableName::try_new(yaml.table)?;
        let db_schema = match yaml.schema {
            Some(s) => kernel::DatabaseSchema::try_new(s)?,
            None => kernel::DatabaseSchema::default(),
        };
