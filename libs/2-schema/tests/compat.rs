...
fn every_frozen_config_still_loads_and_converts() {
    for snapshot in snapshots() {
        let config = schema::load(snapshot.join("flusso.toml"))
...
fn every_frozen_lock_still_decodes() {
    for snapshot in snapshots() {
        let from_lock = schema::load_compiled(snapshot.join("flusso.lock"))
            .unwrap_or_else(|err| panic!("{} lock no longer decodes: {err}", snapshot.display()));
        let from_config = schema::load(snapshot.join("flusso.toml")).unwrap();
        assert_eq!(
