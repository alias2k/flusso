...
| Item | Role |
| --- | --- |
| [`cdc`] | *What changed?* — pluggable change capture: a live stream of positioned [`ChangeEvent`](cdc::ChangeEvent)s, progress confirmed with [`confirm`](cdc::ChangeCapture::confirm) |
...
> ℹ️ **Info** — [`SourceSpec`] is the composition root's gift to the backend: the source crate never sees the assembled `Config`. It gets only the enabled indexes and their schemas, in `kernel` types.
