...
/// durably confirmed. While the watched tables are idle, keepalive WAL ends
/// and filtered-out commits feed the same watermark
/// ([`AckShared::register_confirmed`]), so the slot keeps advancing and the
/// server can recycle WAL generated by unrelated traffic.
pub(crate) fn build(
    client: ReplicationClient,
    ack: Arc<AckShared>,
    sink: Arc<dyn AckSink>,
