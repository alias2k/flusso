...
//!
//! Implements [`sources_core::ChangeCapture`] on top of `pgwire-replication`.
//! [`WalChangeCapture::start`] connects to a replication slot and yields a
//! stream of thin [`Change`](sources_core::Change)s — table name and primary
...
//! - Translates the per-change [`Ack`](sources_core::Ack) into a contiguous LSN
