...
3. **Edit the existing code in place; do not create a parallel new struct/module.** Convert the project's current document type to a `#[derive(FlussoRoot)]` projection (see flusso-query), keeping its name, fields, and `serde` renames; child shapes become `#[derive(FlussoFragment)]`. A second "v2" struct alongside the original is wrong unless the user explicitly asks for one.
