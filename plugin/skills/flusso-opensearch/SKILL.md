...
- **Seeding markers.** Seeded state lives in a hidden `flusso_meta` index, so a restart skips a completed backfill instead of redoing it. The marker is reconciled with the generation index it names on every start: if that index is gone, the marker is retracted, the index recreated empty, and the backfill refills it — so **deleting `{logical}_{hash}_{gen}` between runs is the supported way to force a rebuild of one index**. (The marker can't vouch for the source; a recreated replication slot also triggers a rebuild — see the `flusso-postgres` skill.)
...
Define multiple `[sinks.<name>]` and flusso **fans out** — every document lands in each. No sinks → it falls back to a `stdout` sink (handy to *see* documents while integrating). Full reference: `docs/src/reference/sink-opensearch.md` (every key) and `docs/src/reference/config-toml.md` (fan-out).
