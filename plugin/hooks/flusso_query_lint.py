...
uses `#[derive(FlussoRoot)]` *and* also constructs a `Keyword`/`Text` handle
...
High precision by design: a file with no root derive (hand-written
...
def main():
    ...
    sys.stderr.write(
        ...
        "In a `#[derive(FlussoRoot)]` file every schema field already has a "
