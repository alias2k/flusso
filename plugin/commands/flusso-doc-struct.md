...
3. Emit a child struct for every object/join, recursively — each a **`#[derive(FlussoFragment)]`** with **no** `#[flusso(…)]` attribute. A fragment names no location: `$2` validates it against whatever path it sits at, and recursion reaches its children. Never emit `path = "…"` (removed).
   ...
   - Handles for a child level come from `$2`, not from the child struct: an object chains (`$2::account().tier()`), a `nested` array is a generated scope in the `flusso_<root>_query` module, named for its level (`flusso_$2_query::Orders::status()`, `flusso_$2_query::OrdersItems::quantity()` — the module name uses the snake_case root).
