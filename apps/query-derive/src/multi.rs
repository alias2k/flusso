...
//! must itself be a root `FlussoRoot` (usually via its own `FlussoDocument`
...
//!   `FlussoRoot::physical_index()`, deserializing the source into the
...
pub(crate) fn expand(input: DeriveInput) -> TokenStream {
    ...
    // `quote_spanned` pins the `FlussoRoot` bound to the variant's payload
    // type, so "trait not satisfied" points at the offending variant.
    let targets = variants.iter().map(|(_, ty)| {
        quote_spanned! { ty.span() =>
            (
                <#ty as ::flusso_query::FlussoRoot>::INDEX,
                <#ty as ::flusso_query::FlussoRoot>::SCHEMA_HASH,
            )
        }
    });
    ...
    let arms = variants.iter().map(|(variant, ty)| {
        quote_spanned! { ty.span() =>
            if physical_index == <#ty as ::flusso_query::FlussoRoot>::physical_index() {
