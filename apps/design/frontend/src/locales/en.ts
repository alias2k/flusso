// English base catalog: symbolic key → ICU message. Other languages key off
// the same identifiers; any key missing from them falls back to here.
export const en: Record<string, string> = {
  // topbar
  "topbar.hideSidebar": "Hide sidebar",
  "topbar.showSidebar": "Show sidebar",
  "topbar.tables": "Tables",
  "topbar.visual": "Visual",
  "topbar.code": "Code",
  "topbar.modeHint": "Switch between the visual canvas and the YAML editor",
  "topbar.undo": "Undo (⌘Z)",
  "topbar.redo": "Redo (⇧⌘Z)",
  "topbar.toggleTheme": "Toggle theme",
  "topbar.retestDb": "Re-test the database connection",
  "topbar.dbConnected": "DB connected",
  "topbar.dbOffline": "DB offline",
  "topbar.offlineBanner": "Database not reachable — offline authoring only.",
  "topbar.yaml": "Preview",
  "topbar.validate": "Validate",
  "topbar.save": "Save",
  "topbar.reset": "Reset",
  "topbar.resetHint": "Discard unsaved changes",
  "topbar.unsaved": "Unsaved changes",
  "typeWarn.banner":
    "{n, plural, one {# field maps a column to a sharply different type} other {# fields map columns to sharply different types}}.",
  "typeWarn.fixAll": "Fix all",
  "typeWarn.ignore": "Ignore",
  "diag.requiredDefault": 'required, but its source column "{col}" is nullable — set a default value',
  "diag.typeMismatch": 'type "{ty}" is a sharp change from source column "{col}" ({src}) — values may fail to index',
  "topbar.upToDate": "Up to date",
  "topbar.language": "Language",

  // toasts / status
  "toast.saved": "{n, plural, one {Saved # file} other {Saved # files}}",
  "toast.saveFailed": "Save failed: {err}",
  "toast.validateFailed": "Validate failed: {err}",
  "toast.issues":
    "{n, plural, one {# issue — see the highlighted field} other {# issues — see the highlighted fields}}",
  "toast.dbNotReachable": "Database not reachable",
  "toast.dbConnected": "Database connected",
  "toast.alreadyUpToDate": "Already up to date",
  "toast.diffFailed": "Diff failed: {err}",
  "toast.dbNotReachableErr": "Database not reachable: {err}",
  "toast.schemasMatch": "Schemas match the database",

  // sidebar
  "sidebar.deployment": "Deployment",
  "sidebar.indexes": "Indexes",
  "sidebar.off": "(off)",
  "sidebar.kinds": "Node kinds",
  "sidebar.types": "Field types",
  "sidebar.legend": "Legend",
  "legend.kindRoot": "The index root — the document's primary table.",
  "legend.kindObject": "A nested object grouping columns from the same table.",
  "legend.kindBelongsTo": "This row points at one parent row (foreign key on this table).",
  "legend.kindHasOne": "One child row points back here — a single nested object.",
  "legend.kindHasMany": "Many child rows point back here — an array of objects.",
  "legend.kindManyToMany": "Related rows linked through a junction table.",
  "legend.typeString": "Text and keyword fields — text, keyword, identifier, enum.",
  "legend.typeNumber": "Numbers — integer, long, float, double, decimal…",
  "legend.typeDate": "Dates and timestamps.",
  "legend.typeBoolean": "True / false.",
  "legend.typeUuid": "A UUID identifier.",
  "legend.typeGeo": "A geographic point (latitude / longitude).",
  "sidebar.newIndex": "New index",
  "sidebar.newIndexTitle": "New index",
  "sidebar.newIndexDesc": "Name the index, pick its root table, then choose where its schema file lands.",
  "sidebar.indexName": "index name",
  "sidebar.rootTable": "root table",
  "sidebar.schemaDir": "Directory",
  "sidebar.rootDir": "(root — flusso.toml directory)",
  "sidebar.schemaFile": "Schema file",
  "sidebar.schemaFileHint": "Pick a directory (or type a new one) and a file name.",
  "sidebar.overrideName": "Override the full file name",
  "sidebar.lockSuffix": "Use the standard .schema.yml suffix",
  "sidebar.stepDetails": "Details",
  "sidebar.stepFile": "File",
  "sidebar.create": "Create",

  // Code mode
  "raw.help": "Edits sync into the project as you type — Save (top right) reviews and writes the files.",
  "raw.editorAria": "Raw YAML editor",
  "raw.format": "Format",
  "raw.vimHint": "VIM keybindings",
  "code.problems": "Problems",
  "code.problemsN": "{n, plural, one {# problem} other {# problems}}",
  "code.noProblems": "valid YAML",
  "code.validYaml": "valid YAML",
  "code.shortcuts": "Shortcuts",
  "code.search": "Search",
  "code.fold": "Fold block",
  "code.cursor": "Ln {l}, Col {c}",
  "code.autoFormat": "Auto-format",
  "code.autoFormatHint": "Format the buffer on focus loss whenever the YAML is valid",

  // diff modal
  "diff.aria": "Review changes",
  "diff.reviewTitle": "Review changes",
  "diff.selected": "{n} of {m} selected",
  "diff.filterFiles": "Filter files…",
  "diff.noFiles": "No matching files",
  "diff.kbdNavigate": "Navigate",
  "diff.kbdToggle": "Toggle file",
  "diff.kbdToggleAll": "Toggle all",
  "diff.kbdSave": "Save",
  "diff.kbdClear": "Clear filter",
  "diff.newFile": "New",
  "diff.opMove": "Move",
  "diff.opDelete": "Delete",
  "diff.movedFrom": "Moved from {path}",
  "diff.warnOutside": "This path resolves outside the config directory.",
  "diff.write": "{n, plural, one {Write # file} other {Write # files}}",
  "diff.unchanged": "{n, plural, one {# unchanged line} other {# unchanged lines}}",
  "diff.validating": "Validating…",
  "diff.allGood": "All checks passed",
  "diff.dbOffline": "Database offline",
  "diff.validateFailed": "Validation failed",
  "diff.issues": "{n, plural, one {# issue found} other {# issues found}}",
  "diff.viewUnified": "Unified",
  "diff.viewSplit": "Split",
  "diff.viewOld": "Old",
  "diff.viewNew": "New",

  // shared
  "common.cancel": "Cancel",
  "common.next": "Next",
  "common.back": "Back",
  "common.remove": "remove",
  "common.clear": "Clear",
  "common.close": "Close",
  "common.column": "column",
  "common.noMatch": "No match",
  "common.useValue": 'Use "{v}"',
  "common.loading": "Loading project…",

  // inspector
  "inspector.selectPrompt": "Select a node or field to edit its details.",
  "inspector.indexRoot": "Index root",
  "inspector.rootTable": "root table",
  "inspector.schema": "schema",
  "inspector.rootFilters": "root filters",
  "inspector.objectGroup": "Object group",
  "inspector.duplicate": "Duplicate",
  "inspector.delete": "Delete",
  "inspector.close": "Close",
  "inspector.columnsSelected": "{n, plural, one {# row selected} other {# rows selected}}",
  "inspector.includeSelected": "{n, plural, one {Include # column} other {Include # columns}}",
  "inspector.removeSelected": "{n, plural, one {Remove # row} other {Remove # rows}}",
  "inspector.groupHint": "A group nests columns of the same table. Add fields on its node.",
  "inspector.join": "Join",
  "inspector.field": "Field",
  "inspector.fieldName": "field name",
  "inspector.verb": "verb",
  "inspector.table": "table",
  "inspector.btColumn": "column (this table → target)",
  "inspector.fkOnTarget": "foreign_key (on target)",
  "inspector.fkNullable": "FK column {col} is nullable — the target may be absent, so this join is optional.",
  "inspector.fkNotNull": "FK column {col} is NOT NULL — the target is always present.",
  "inspector.filters": "filters",
  "inspector.valueJson": "value (JSON)",
  "inspector.required": "required",
  "inspector.fromSource": "from source",
  "inspector.type": "type",
  "inspector.use": "use",
  "inspector.lowercase": "lowercase",
  "inspector.trim": "trim",
  "inspector.variants": "Variants (ordered)",
  "inspector.variantsHint":
    "List the values in rank order — the field sorts by this order instead of alphabetically. Leave empty for a plain enum.",
  "inspector.variantsAdd": "Add a variant…",
  "inspector.add": "Add",
  "inspector.variantUp": "Move up",
  "inspector.variantDown": "Move down",
  "inspector.variantRemove": "Remove variant",
  "inspector.defaultRequired": "default (required)",
  "inspector.defaultOptional": "default (optional, JSON)",
  "inspector.defaultPlaceholder": 'e.g. 0 or "n/a"',
  "inspector.colNotNull": "NOT NULL",
  "inspector.colNullable": "nullable",
  "inspector.suggested": "suggested",
  "inspector.typeMismatch": "{ty} is a sharp change from the source column ({col}) — values may fail to index.",
  "typeDesc.text": "Analyzed full-text search",
  "typeDesc.identifier": "Exact ID; not analyzed",
  "typeDesc.keyword": "Exact match; filter & sort",
  "typeDesc.enum": "Fixed set of values",
  "typeDesc.uuid": "UUID, stored as keyword",
  "typeDesc.boolean": "True or false",
  "typeDesc.short": "16-bit integer",
  "typeDesc.integer": "32-bit integer",
  "typeDesc.long": "64-bit integer",
  "typeDesc.float": "32-bit floating point",
  "typeDesc.double": "64-bit floating point",
  "typeDesc.decimal": "Exact decimal number",
  "typeDesc.date": "Calendar date",
  "typeDesc.timestamp": "Date and time",
  "typeDesc.binary": "Base64-encoded bytes",
  "typeDesc.json": "Arbitrary JSON object",
  "inspector.fromDb": "From the database",
  "inspector.inDoc": "In the document",
  "inspector.aggFrom": "Aggregated from",
  "inspector.relationship": "The relationship",
  "inspector.advanced": "Advanced · index tuning",
  "inspector.renameTo": "rename to",
  "inspector.noSource": "No database source — a fixed value in every document.",
  "inspector.nestedNote": "↳ a nested object — choose its columns & sub-joins on the node.",
  "inspector.countResult": "Counts the related rows into an integer.",
  "inspector.bridgeNotNull": "NOT NULL → required by default. Uncheck to make it optional in the document.",
  "inspector.bridgeNullable": "Nullable → optional by default. Mark it required and a default becomes mandatory.",
  "inspector.defaultError":
    "A required field over a nullable column must set a default, or the document field could be missing.",
  "inspector.optionsHelp":
    "Extra OpenSearch mapping properties merged beside the derived type. Pick a common one or add your own. Each value is JSON.",
  "inspector.optKey": "key",
  "inspector.optValue": 'value (JSON, e.g. "english" or 2)',
  "inspector.option": "option",
  "inspector.values": "values",
  "inspector.columnJson": "column (json/jsonb)",
  "inspector.pgTypes": "postgres types (comma)",
  "inspector.osType": "opensearch type",
  "inspector.latColumn": "lat column",
  "inspector.lonColumn": "lon column",
  "inspector.geoHint":
    "A point is absent when either column is null (never sent as '{'lat:null'}'). Mark required to forbid that absence.",
  "inspector.relatedTable": "related table",
  "inspector.aggColumn": "column (to aggregate)",
  "inspector.junctionTable": "junction table",
  "inspector.softDelete": "soft delete",
  "inspector.documentField": "document field",

  // grammar one-liners (by field/join kind)
  "kindHelp.belongs_to": "This row points at one target row (single nested object).",
  "kindHelp.has_one": "One related row points back here (single nested object).",
  "kindHelp.has_many": "Many related rows point back here (array of objects).",
  "kindHelp.many_to_many": "Related rows linked through a junction table (array of objects).",
  "kindHelp.object": "Groups columns of the same table under a nested object — no new table.",
  "kindHelp.count": "Counts the related rows into a number.",
  "kindHelp.sum": "Sums a column of the related rows.",
  "kindHelp.avg": "Averages a column of the related rows (result is a double).",
  "kindHelp.min": "Smallest value of a column across the related rows.",
  "kindHelp.max": "Largest value of a column across the related rows.",
  "kindHelp.ids": "Collects the related table's primary keys into an array.",
  "kindHelp.geo": "A geo point from two columns (lat/lon).",
  "kindHelp.map": "A dynamic-key object over a json/jsonb column; keys stay searchable.",
  "kindHelp.custom": "A type flusso doesn't model — you give the Postgres + OpenSearch types.",
  "kindHelp.constant": "A fixed value baked into every document.",

  // preview
  "preview.title": "Preview",
  "preview.empty": "Select or edit an index to preview it.",
  "preview.parseError": "This schema does not parse:",
  "preview.document": "Document",
  "preview.tabYaml": "YAML",
  "preview.tabMapping": "Mapping",
  "preview.tabSample": "Sample",
  "preview.copy": "copy",
  "preview.selectAll": "select all",
  "preview.copied": "copied",
  "preview.example": "example",
  "preview.building": "building…",
  "preview.refresh": "refresh",
  "preview.fetch": "Build sample",
  "preview.sampleTitle": "Sample document",
  "preview.sampleHint": "Build a document from a live database row — exactly what the sink writes.",

  // catalog browser
  "catalog.title": "{n, plural, one {Database (# table)} other {Database (# tables)}}",
  "catalog.dbError": "Database not reachable — {err}",
  "catalog.filter": "Filter tables & columns…",
  "catalog.junction": "junction",
  "catalog.cols": "{n, plural, one {# col} other {# cols}}",
  "catalog.pk": "primary key",
  "catalog.matchCount": "{n, plural, one {# table} other {# tables}}",
  "catalog.referencedBy": "Referenced by",
  "catalog.noMatch": "No matching tables or columns",

  // canvas node (DocNodeView)
  "node.expand": "Expand",
  "node.collapse": "Collapse",
  "node.renameHint": "Double-click to rename",
  "node.renameField": "Field name",
  "node.pk": "pk",
  "node.fields": "{n, plural, one {# field} other {# fields}}",
  "node.diagCount":
    "{n, plural, one {# field disagrees with the database} other {# fields disagree with the database}}",
  "node.joinIncomplete": "This join is missing a required key — set it in the inspector",
  "node.pickRoot": "Pick a root table to begin",
  "node.chooseTable": "choose a table…",
  "node.rootTableEnter": "root table name, Enter",
  "node.filterCols": "filter columns…",
  "node.includeAll": "include all columns",
  "node.clearAll": "clear all columns",
  "node.columnOf": "column: {name}",
  "node.reqAligned": "required (the column is NOT NULL)",
  "node.reqOverride": "required — overrides a nullable column",
  "node.colDefault": "has a default value",
  "node.incomplete": "incomplete — set its key/column in the inspector",
  "node.addJoin": "+ join",
  "node.addField": "+ field",
  "node.addColumn": "+ column name, Enter",
  "node.fieldGroup": "fields",
  "node.aggGroup": "aggregates",
  "node.addRelation": "{n, plural, one {# suggested relation} other {# suggested relations}}",
  "node.addRelationHint": "Pull in a related table as a join — search the foreign keys pointing to and from this table",
  "node.searchRelations": "search relations…",
  "node.searchFields": "search fields…",

  // config panel
  "config.indexPrefix": "index prefix",
  "config.none": "(none)",
  "config.source": "Source",
  "config.stageIndexesHint": "synced documents",
  "config.stageSinksHint": "destinations",
  "config.backfill": "Backfill",
  "config.backfillHint": "Seed unseeded indexes from a snapshot. Off: live changes only, for a stateless sink.",
  "config.sinks": "Sinks",
  "config.sink": "sink",
  "config.name": "name",
  "config.schemaFile": "schema file",
  "config.onError": "on error",
  "config.state": "state",
  "config.enabled": "enabled",
  "config.disabled": "disabled",
  "config.duplicate": "duplicate",
  "config.index": "index",
  "config.removeIndexTitle": "Remove index?",
  "config.removeIndex": 'Remove index "{name}"? (the schema file is left on disk)',
  "config.connection": "connection",
  "config.stream": "Stream",
  "config.streamHint": "between the source and the sinks",
  "config.options": "options",
  "config.required": "required",
  "config.default": "default",
  "config.literal": "literal",
  "config.env": "env var",
  "config.parts": "parts",
  "config.loadingAdapters": "loading adapters…",
  "config.unknownAdapter": 'No adapter is registered for type "{kind}"',

  // filters
  "filters.value": "value",
  "filters.rangeFrom": "from",
  "filters.rangeTo": "to",
  "filters.and": "and",
  "filters.filter": "filter",
  "filters.kindRaw": "Raw SQL",
  "filters.kindRawDesc": "Verbatim SQL condition",
  "filters.kindNullCheck": "Null check",
  "filters.kindNullCheckDesc": "Column is (not) null",
  "filters.kindValueOp": "Column comparison",
  "filters.kindValueOpDesc": "Compare a column to a value",
  "filters.opEq": "equals",
  "filters.opNeq": "not equal",
  "filters.opLt": "less than",
  "filters.opLte": "less than or equal",
  "filters.opGt": "greater than",
  "filters.opGte": "greater than or equal",
  "filters.opIn": "in a list",
  "filters.opNotIn": "not in a list",
  "filters.opLike": "pattern match",
  "filters.opIlike": "case-insensitive match",
  "filters.opBetween": "within a range",
  "filters.opIsNull": "no value",
  "filters.opIsNotNull": "has a value",
  "filters.rawPlaceholder": "e.g. status <> 'archived'",
  "filters.help":
    "Restrict which rows feed this field — a raw SQL condition, a null check, or a column/operator/value test. All must hold.",

  // canvas controls
  "canvas.zoomIn": "Zoom in",
  "canvas.zoomOut": "Zoom out",
  "canvas.fitView": "Fit to view",
  "canvas.lock": "Lock layout",
  "canvas.unlock": "Unlock layout",
  "canvas.resetLayout": "Reset layout",
  "canvas.showMinimap": "Show minimap",
  "canvas.hideMinimap": "Hide minimap",
  "canvas.collapseAll": "Collapse all",
  "canvas.expandAll": "Expand all",

  // global search / command palette
  "search.placeholder": "Search everything…",
  "search.empty": "No results",
  "search.emptyDetail": "Select a result to preview it.",
  "search.resultCount": "{n, plural, one {# result} other {# results}}",
  "search.title": "Global search",
  "search.actions": "Actions",
  "search.indexes": "Indexes",
  "search.fields": "Fields",
  "search.settings": "Settings",
  "search.legendKinds": "Legend · Node kinds",
  "search.legendTypes": "Legend · Field types",
  "search.tables": "Tables & columns",
  "search.reference": "Reference",
  "search.undo": "Undo",
  "search.redo": "Redo",
  "search.toVisual": "Switch to Visual",
  "search.toCode": "Switch to Code",
  "search.serverAddresses": "Server addresses",
  "search.navigate": "navigate",
  "search.selectHint": "select",
  "search.closeHint": "close",
  "search.complete": "complete",
  "search.recent": "Recent",
  "search.removeRecent": "Remove from recent",
  "search.onScreenFirst": "on-screen results rank first",
  "search.runAction": "Run action",
  "search.openIndex": "Open index",
  "search.jumpField": "Jump to field · pans the canvas",
  "search.jumpNode": "Jump to node · pans the canvas",
  "search.openSettings": "Open in deployment settings",
  "search.browseTables": "Open the table browser",
  "search.descValidate": "Check schemas against the database",
  "search.descSave": "Write schema & config changes to disk",
  "search.descReset": "Discard unsaved changes",
  "search.descDeployment": "Edit connection, sinks & options",
  "search.descTables": "Open the database catalog",
  "search.descYaml": "Toggle the schema & document preview",
  "search.descTheme": "Switch between dark and light",
  "search.descSidebar": "Show or hide the index list",
  "search.descNewIndex": "Create a new index",
  "search.descUndo": "Undo the last change",
  "search.descRedo": "Redo the last undone change",
  "search.descMode": "Switch between the visual canvas and the YAML editor",
  "search.descPrefix": "A prefix prepended to every index name.",
  "search.descConnection": "The Postgres source connection.",
  "search.descOnError": "Global policy when a document fails to index.",
  "search.descServer": "HTTP addresses for metrics, status & admin.",
  "search.descSink": "An output destination (OpenSearch or stdout).",
};
