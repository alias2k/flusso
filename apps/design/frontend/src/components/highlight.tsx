...
// A tiny, dependency-free syntax highlighter for the designer's YAML and JSON,
...
// machine-generated or hand-edited schema text, so a line/token regex is
// enough; it never needs to parse arbitrary YAML/JSON. Shared by the read-only
// CodeBlock and the raw-YAML editor's paint layer, so both render identically.
...
/// One highlighted YAML line. Emits exactly the input's characters (only
/// wrapped in spans), so an overlay textarea's glyphs align 1:1.
export function yamlLine(line: string): ReactNode {
...
/// One highlighted JSON line (same character-preserving contract as [`yamlLine`]).
export function jsonLine(line: string): ReactNode {
  const out: ReactNode[] = [];
  let last = 0;
  JSON_RE.lastIndex = 0;
