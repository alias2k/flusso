...
import { PanelTitle } from "./widgets";
...
/// A read-only browser of the introspected database, shown as a main view (like
/// Deployment): a table list on the left, the selected table's columns +
/// relationships on the right. Foreign keys are shown inline on their column
/// (→ target), incoming references in a footer. The filter spans table *and*
/// column names — a column hit surfaces its table.
export function CatalogBrowser({ catalog }: { catalog: CatalogResponse }) {
  const { t } = useT();
  ...
  return (
    <div className="flex h-full min-h-0 flex-col p-4">
      <PanelTitle>{t("catalog.title", { n: tables.length })}</PanelTitle>

      {catalog.error ? (
        <p className="banner warn">{t("catalog.dbError", { err: catalog.error })}</p>
      ) : (
        <>
          <div className="mb-3 flex h-9 items-center gap-2.5 rounded-md border border-border bg-secondary px-3 focus-within:border-primary">
          ...
          <div className="grid min-h-0 flex-1 grid-cols-[16rem_1fr] overflow-hidden rounded-lg border border-border">
