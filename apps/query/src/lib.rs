...
/// One glob import for everyday querying: `use flusso_query::prelude::*;`.
///
/// Pulls in the traits you need in scope ([`AsQuery`] for `.and`/`.or` on
/// builders, [`FlussoDocument`](trait@FlussoDocument)), the entry points
/// ([`Client`], [`Search`]), every field handle, the typed query/sort
/// parameters (the option enums, [`Distance`], [`MinimumShouldMatch`], …), the
/// standalone query free functions, and — with the `derive` feature — the
/// `#[derive(...)]` macros. The per-query builder structs (`MatchQuery`,
/// `RangeQuery`, …) are intentionally left out — you get them from a handle
/// method, you rarely name them — import them from the crate root if needed.
///
/// ```
/// use flusso_query::prelude::*;
///
/// // `AsQuery` (for `.or` on a builder), the handles, and the option enums
/// // are all in scope from the one glob.
/// let q = Keyword::<Root>::at("status")
///     .any_of(["paid", "shipped"])
///     .or(Number::<i64, Root>::at("total").gte(100));
/// let _ = q.to_value();
/// ```
pub mod prelude {
    // `FlussoDocument`/`FlussoValue`/`FlussoMap`/`FlussoMultiDocument` each name
    // both a trait and (with the `derive` feature) a same-named derive macro;
    // one `use` brings both, since they live in different namespaces.
    pub use crate::{
