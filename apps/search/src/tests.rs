...
fn combinators_build_bool_clauses() {
    let and = Keyword::at("a").eq("x").and(Keyword::at("b").eq("y"));
    assert_eq!(
        and.to_value(),
        json!({ "bool": { "must": [
            { "term": { "a": "x" } },
    ...
    let chained = Keyword::at("a")
        .eq("x")
        .and(Keyword::at("b").eq("y"))
        .and(Keyword::at("c").eq("z"));
    assert_eq!(
        chained.to_value(),
        json!({ "bool": { "must": [
            { "term": { "a": "x" } },
    ...
    let or = Keyword::at("a").eq("x").or(Keyword::at("b").eq("y"));
    assert_eq!(
        or.to_value(),
        json!({ "bool": { "should": [
            { "term": { "a": "x" } },
    ...
    let negated = Keyword::at("a").eq("x").not();
    assert_eq!(
        negated.to_value(),
...
fn operators_render_expected_clauses() {
    assert_eq!(
        Keyword::at("status").in_(["paid", "shipped"]).to_value(),
        json!({ "terms": { "status": ["paid", "shipped"] } })
    );

    assert_eq!(
        Keyword::at("email").prefix("test-").to_value(),
        json!({ "prefix": { "email": "test-" } })
    );

    assert_eq!(
        Number::<i64>::at("n").between(1, 10).to_value(),
        json!({ "range": { "n": { "gte": 1, "lte": 10 } } })
    );

    assert_eq!(
        Text::at("bio").exists().to_value(),
        json!({ "exists": { "field": "bio" } })
    );

    // `all` is "no element fails the predicate".
    let all = Nested::<()>::at("orders").all(Keyword::at("orders.status").eq("paid"));
    assert_eq!(
        all.to_value(),
        json!({ "bool": { "must_not": [
            { "nested": {
...
fn extended_term_and_text_operators() {
    assert_eq!(
        Keyword::at("sku").wildcard("C-?23*").to_value(),
        json!({ "wildcard": { "sku": "C-?23*" } })
    );

    assert_eq!(
        Keyword::at("sku").regexp("c-[0-9]+").to_value(),
        json!({ "regexp": { "sku": "c-[0-9]+" } })
    );

    assert_eq!(
        Keyword::at("city").fuzzy("bostn").to_value(),
        json!({ "fuzzy": { "city": "bostn" } })
    );

    assert_eq!(
        Text::at("bio").match_phrase_prefix("software eng").to_value(),
        json!({ "match_phrase_prefix": { "bio": "software eng" } })
    );

    assert_eq!(
        Text::at("bio").matches_fuzzy("enginer").to_value(),
...
fn multi_match_spans_text_fields() {
    let query = multi_match("ada lovelace", [Text::at("fullName"), Text::at("bio")]);
    assert_eq!(
        query.to_value(),
        json!({ "multi_match": {
            "query": "ada lovelace",
...
fn geo_queries_render_expected_clauses() {
    ...
    assert_eq!(
        Geo::at("location").within("10km", here).to_value(),
        json!({ "geo_distance": {
            "distance": "10km",
...
fn geo_distance_sort_in_search_body() -> Result {
    ...
    let body = Search::<User>::new(&client, "places")
        .sort(Geo::at("location").distance_sort(GeoPoint::new(52.37, 4.90), SortOrder::Asc, "km"))
        .body();
