-- The replication objects storno requires to already exist.
--
-- These names match the CLI defaults (`--slot storno`, `--publication storno`).
-- The publication must cover every table any index reads from — here that is
-- the `users` root and its joined `orders` child.

CREATE PUBLICATION storno FOR TABLE users, orders;
...
SELECT pg_create_logical_replication_slot('storno', 'pgoutput');
